...
import com.kelompok4.serena.ui.components.VideoCard
import com.kelompok4.serena.ui.screens.VideoData
...
fun SelfCareScreen(
    ...
    LazyColumn(
        ...
        item {
            ...
            SectionHeader(
                title = "Video",
                onClickSeeAll = { /* Optional: Navigate to video list */ }
            )
            LazyRow(
                contentPadding = androidx.compose.foundation.layout.PaddingValues(horizontal = 16.dp)
            ) {
                items(VideoData.videos) { video ->
                    VideoCard(
                        painterRes = video.thumbnail,
                        title = video.title,
